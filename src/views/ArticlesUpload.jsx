...
function ArticlesUpload() {
  ...
  const [articleUploadError, setArticleUploadError] = React.useState(null);
  const [articleUploadSuccess, setArticleUploadSuccess] = React.useState(false);
  ...
  const handleSubmit = async (e) => {
    ...
    // Basic validation
    if (!title) {
      setArticleUploadError("Please enter a story title.");
      return;
    }
    if (!content) {
      setArticleUploadError("Please enter story content.");
      return;
    }
    ...
    try {
      ...
      const { error: uploadErrorRes } = await supabase.storage
      ...
      // 2. Insert the article data (title, subtitle, description, content, image_url) into your database table
      const { data: articleData, error: articleInsertError } = await supabase
        .from("articles") // IMPORTANT: Replace 'articles' with your actual table name for articles
        .insert([
          {
            title: title,
      ...
      if (articleInsertError) {
        throw articleInsertError;
      }

      setUploadSuccess(true);
      setArticleUploadSuccess(true);
      setTitle(""); // Clear form fields on success
      setSubtitle(""); // Clear subtitle
      setDescription(""); // Clear description
      setContent("");
      setFile(null); // Clear selected file
      console.log("Article uploaded successfully:", articleData);
    } catch (error) {
      console.error("Error during upload process:", error);
      // Differentiate between file upload error and article insert error
      if (error.message && error.message.includes("upload")) {
        setUploadError(`Image upload failed: ${error.message}`);
      } else {
        setArticleUploadError(`Article submission failed: ${error.message}`);
      }
    } finally {
      setUploading(false);
    }
    ...
    <div className="flex">
      ...
      <div className="ml-5 p-6 flex-grow">
        ...
        >
          ...
          {articleUploadError && (
            <p className="text-red-600 mt-2">Error: {articleUploadError}</p>
          )}
          {uploadSuccess && articleUploadSuccess && (
            <p className="text-green-600 mt-2">Article uploaded successfully!</p>
          )}
        </form>
      </div>
    </div>
